...
export default function Input({ items, setItems, filtered, setFiltered, getFilterFromSelect, isDefaultItems }) {

  const [input, setInput] = React.useState('')
  const [newItemId, setNewItemId] = React.useState(isDefaultItems? isDefaultItems : 1)
  ...
  const handleSwitch = (e) => {
    ...
    getFilterFromSelect(e.target.value)
  }
  ...
  const clear = (e) => {
    ...
    <section className='input__section flex justify-center py-2 mt-[50px] mb-[70px]'>
      <form onSubmit={addTask} style={{display:"flex", justifyContent:"center"}}>
        ...
        <button onClick={addTask} type='submit' className='w-8 cursor-pointer border bg-white flex justify-center items-center border-r-gray-200'>
      ...
      <button onClick={clear} className='w-[100px] ml-2 cursor-pointer border bg-red-600 flex justify-center items-center border-r-gray-200'>
        Clear
      </button>
